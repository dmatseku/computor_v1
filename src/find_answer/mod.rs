...
pub fn square_answer(formula: &Formula) {
    let a: f32 = match formula.get_left_side().iter().find(|x| x.get_power() == 2) {
        Some(t) => t.get_coefficient(),
        None => 0.0,
    };
    let b: f32 = match formula.get_left_side().iter().find(|x| x.get_power() == 1) {
        Some(t) => t.get_coefficient(),
        None => 0.0,
    };
    let c: f32 = match formula.get_left_side().iter().find(|x| x.get_power() == 0) {
        Some(t) => t.get_coefficient(),
        None => 0.0,
    };

...
impl Formula {
	...
	fn complete_vec(&mut self, formula_sub_string: &str, side: bool) { //create one side
		let re = Regex::new(
		...
		for str in caps { //add all operands to the vector
			...
			match self.left_side.iter_mut().find(|x| x.get_power() == new_mon.get_power()) {
				Some(t) => {
	...
	pub fn new(formula_string: &str) -> Result<Formula, errors::SyntaxError> {
		let mut res: Formula = Formula { left_side: Vec::new() };
		...
		while match it.next() {
			Some(t) => {
				...
				if first {
					...
					print!("{}", if t.get_coefficient() < 0.0 { "- " } else { "+ " });
				}
